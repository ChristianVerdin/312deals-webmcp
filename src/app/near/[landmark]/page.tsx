...
import { MapPin, Store, Tag, HelpCircle, ExternalLink } from "lucide-react"
...
async function getDeals(config: (typeof LANDMARK_PAGES)[string]): Promise<Deal[]> {
  try {
    const allDeals: Deal[] = []
    const seenIds = new Set<number>()

    for (const neighborhood of config.neighborhoods) {
      const params = new URLSearchParams({ neighborhood, limit: "50" })
...
export async function generateMetadata({
  ...
  const config = LANDMARK_PAGES[params.landmark]
  if (!config) return { title: "Near | 312Deals" }

  const ogParams = new URLSearchParams({
    title: config.seoTitle,
    subtitle: config.description.slice(0, 100),
    type: config.landmarkName,
  })
  const ogImageUrl = `https://www.312deals.com/api/og?${ogParams}`

  // Soft-404 guard (mirrors student-guides/cuisine): emit noindex when the
  ...
  return {
    ...
    openGraph: {
      title: `${config.seoTitle} | 312Deals`,
      description: config.description,
      url: `https://www.312deals.com/near/${params.landmark}`,
      siteName: "312Deals",
      type: "website",
    ...
    alternates: {
      canonical: `https://www.312deals.com/near/${params.landmark}`,
    },
  }
}

export default async function NearLandmarkPage({
  params,
}: {
  params: { landmark: string }
}) {
  ...
  const faqItems = deals.length > 0
    ? [
        {
          q: `Where can I eat and drink near ${config.landmarkName}?`,
          a: `There are ${deals.length} food and drink deal${deals.length !== 1 ? "s" : ""} at ${venueCount} venue${venueCount !== 1 ? "s" : ""} within walking distance of ${config.landmarkName}. ${topVenues.length > 0 ? `Popular spots include ${topVenues.join(", ")}.` : ""}`,
        },
        {
          q: `What are the best bars near ${config.landmarkName}?`,
          a: deals.slice(0, 3).map((d) => `${d.venue_name} offers ${d.title}`).join(". ") + ".",
        },
        ...(cheapest
          ? [{
              q: `What's the cheapest drink deal near ${config.landmarkName}?`,
  ...
  const otherLandmarks = Object.entries(LANDMARK_PAGES).filter(
    ([slug]) => slug !== params.landmark
  )
      ...
      <div className="flex-1">
        <div className="mx-auto max-w-7xl px-4 py-8 lg:px-6">
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify(
                buildBreadcrumbJsonLd([
                  ...
                  { name: "Near", url: "https://www.312deals.com/near" },
                  { name: config.landmarkName, url: `https://www.312deals.com/near/${params.landmark}` },
                ])
              ),
            }}
          />
              ...
              <script
                ...
                dangerouslySetInnerHTML={{
                  __html: JSON.stringify(
                    buildItemListJsonLd(
                      config.seoTitle,
                      `https://www.312deals.com/near/${params.landmark}`,
                      deals
                    )
                  ),
                }}
          ...
          <nav className="mb-4 flex items-center gap-1.5 text-sm text-muted-foreground">
            <Link href="/near" className="hover:text-foreground transition-colors">
              Near
            </Link>
            <span>/</span>
            <span className="text-foreground">{config.landmarkName}</span>
          </nav>
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                ...
                "image": {
                  "@type": "ImageObject",
                  "url": `https://www.312deals.com/api/og?${new URLSearchParams({ title: config.seoTitle, subtitle: config.description.slice(0, 100), type: config.landmarkName })}`,
                  "width": 1200,
                  "height": 630,
                },
          ...
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              By <span className="font-medium text-foreground">312Deals Team</span> · Updated{" "}
            ...
            {config.externalLinks.length > 0 && (
              <p className="mt-2 text-sm text-muted-foreground">
                ...
                {config.externalLinks.map((link, i) => {
                  const internal = link.url.startsWith("https://www.312deals.com")
                  return (
                    <span key={link.url}>
                      {i > 0 && ", "}
                      {internal ? (
                        <Link
                          href={link.url.replace("https://www.312deals.com", "")}
          ...
          {config.neighborhoods.length > 0 && (
            <div className="mt-12">
              <h2 className="mb-4 text-lg font-semibold text-foreground">
                Neighborhoods Near {config.landmarkName}
              </h2>
          ...
          <div className="mt-8">
            <h2 className="mb-4 text-lg font-semibold text-foreground">
              More Spots Near Chicago Venues
            </h2>
            <div className="flex flex-wrap gap-2">
              {otherLandmarks.map(([slug, c]) => (
                <Link
                  key={slug}
                  href={`/near/${slug}`}
                  className="rounded-full border border-border bg-card px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:border-brand-300 hover:text-brand-600"
